...
__global__ void sumMatrix(float *MatA, float *MatB, float *MatC, int nx, int ny)
{
    ...
    int iy = threadIdx.x + blockDim.y * blockIdx.y;
    int idx = ix + iy * ny;
    if (ix < nx && iy < ny)
    {
...
int main(int argc, char **argv)
{
    ...
    printf("CPU execution configuration<<<(%d,%d),(%d,%d)>>> Time elapsed %f sec\n",
           grid_0.x, grid_0.y, block_0.x, block_0.y, elapes);
    ...
    printf("GPU Execution configuration<<<(%d,%d),(%d,%d)>>> Time elapsed %f sec\n",
           grid_1.x, grid_1.y, block_1.x, block_1.y, elapes);
    CHECK(cudaMemcpy(C_from_gpu, C_dev, nBytes, cudaMemcpyDeviceToHost));
    checkResult(C_host, C_from_gpu, nxy);
    ...
    elapes = cpuSecond() - start;
    printf("GPU Execution configuration<<<(%d,%d),(%d,%d)>>> Time elapsed %f sec\n",
           grid_2.x, grid_2.y, block_2.x, block_2.y, elapes);
    CHECK(cudaMemcpy(C_from_gpu, C_dev, nBytes, cudaMemcpyDeviceToHost));
    checkResult(C_host, C_from_gpu, nxy);
